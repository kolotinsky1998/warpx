...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Animate electron-density maps from rho_e_*.txt files."
    )
    parser.add_argument(
        "--pattern",
        default="rho_e_*.txt",
...
def main() -> None:
    ...
    ax.set_title("Electron Density Evolution", fontsize=22, pad=14)
    ...
    cbar.set_label(r"$|\rho_e|$ (arbitrary units)", fontsize=14)
    ...
    def update(frame_idx: int):
        ...
        stat_text.set_text(
            f"max |rho_e| = {frame_abs.max():.3e}\nmean |rho_e| = {frame_abs.mean():.3e}"
        )
